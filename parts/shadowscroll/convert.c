...
int find_char(ctx* ctx, unsigned char* charset, unsigned char* block, int pos) {
    ...
    //if (pos >= ctx->reserve_from && pos < ctx->reserve_from + ctx->reserved) return pos;
    for (y = 0; y < ctx->tile_height; y++) {
        if (res == pos) return pos;
        if (res >= ctx->reserve_from && res < ctx->reserve_from + ctx->reserved) {
            //skip char, as it is reserved
            y = -1; res++;
	} else if (charset[res * ctx->tile_height + y] != block[y]) {
...
void next_charset(ctx* ctx, unsigned char* charset, int charpos) {
    if (charpos == 0) return;
    printf("charset %d contains %d chars (+from %d to %d reserved).\n",ctx->cs_num, charpos - ctx->reserved, ctx->reserve_from, ctx->reserve_from + ctx->reserved);
...
void convert(ctx* ctx, unsigned char* data, unsigned char* colormap, unsigned char* charmap, unsigned char* charset) {
    ...
    for (by = ctx->y_start; by <= ctx->y_end - ctx->tile_height; by += ctx->tile_height) {
        ...
        if (ctx->fix && new_charset) {
            ...
            memset(charpos * 8 + charset + ctx->cs_num * ctx->cset_size + 8, 255, ctx->tile_height);
	    ...
	    // skip first x chars if reserved
            if (charpos == ctx->reserve_from) charpos += ctx->reserved;
        ...
        if (ctx->num_splits) {
            ...
            for (s = 0; s < ctx->num_splits; s++) {
                if (by + ctx->tile_height == ctx->splits[s]) {
                    ...
                    charpos = 0;
                    new_charset = 1;
                    printf("user defined split at y=%d\n",ctx->splits[s]);
                }
            }
        }
...
int main(int argc, char *argv[]) {
	...
	fprintf(stderr, "\t-e     convert to ecm mode\n");
        fprintf(stderr, "\t-r     reserve chars from here (default = 0)\n");
        fprintf(stderr, "\t-n     num of chars to be reserved\n");
	...
	fprintf(stderr, "\t-m     map another file to same\n");
        exit (2);
    }
    while (++argv, --argc) {
        sw = *argv;
        if (argc >= 2 && !strcmp(*argv, "-f")) {
            ...
            argc--;
        } else if (argc >= 2 && !strcmp(*argv, "-n")) {
            ctx.reserved = read_number(sw, *++argv, 0, 256);
            argc--;
        } else if (argc >= 2 && !strcmp(*argv, "-r")) {
            ctx.reserve_from = read_number(sw, *++argv, 0, 256);
            argc--;
    ...
    for (x = 0; x < ctx.width / 8; x++) {
        fputc(0, fw);
        fputc(0, fw);
        for (y = 0; y < ctx.height / 8; y++) {
            c = charmap[y * ctx.width / 8 + x];
            fputc(c, fw);
        }
    }
    fclose(fw);
