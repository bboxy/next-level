...
function smallweight(x,y, px, py){

	var dist = 1/((px-x)*(px-x) + (py-y)*(py-y))
	var factor = 0.012*192000;
	var w = factor*dist
	w = Math.floor(w);
	if(w >127){
		w = 127
...
for(var y = 0;y<256;y+=8){
	for(var x = 0;x<256;x+=8){
		var wt0 = smallweight(x,y,p1x,p1y)+smallweight(x,y,p2x,p2y);
		wt0 = normalize(wt0);
		var wt1 = smallweight(x+7,y,p1x,p1y)+smallweight(x+7,y,p2x,p2y) ;
		wt1 = normalize(wt1);
		var wt2 = smallweight(x,y+7,p1x,p1y)+smallweight(x,y+7,p2x,p2y) ;
		wt2 = normalize(wt2);
		var wt3 = smallweight(x+7,y+7,p1x,p1y)+smallweight(x+7,y+7,p2x,p2y) ;
		wt3 = normalize(wt3);
...
lda $2000,x //4
adc $2000,y //4
clc...rol // set lowest bit, clear carry //2
sta !p1+ +2 //4
lda $2000,x //4
adc $2000,y //4
